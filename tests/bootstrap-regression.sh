...
assert_file_contains docker/nginx/default.conf 'client_max_body_size 2m;'
assert_file_contains docker/nginx/default.conf 'location = /locale'
...
assert_function_contains run_upgrade bootstrap.sh 'secure_app_config_permissions'
...
assert_function_order run_upgrade bootstrap.sh 'sync_geoip_defaults' 'secure_app_config_permissions'
assert_function_order run_upgrade bootstrap.sh 'docker compose up -d scheduler' 'update_geoip_database'
assert_function_order run_upgrade bootstrap.sh 'wait_for_service_ready scheduler 300' 'update_geoip_database'
assert_function_order run_upgrade bootstrap.sh 'update_geoip_database' 'restart_service_bounded nginx 45 10'
...
assert_function_contains secure_app_config_permissions install.sh 'chown 0:33 "$APP_CONFIG_FILE"'
assert_function_contains secure_app_config_permissions install.sh 'chmod 0640 "$APP_CONFIG_FILE"'
assert_function_contains secure_app_config_permissions bootstrap.sh 'chown 0:33 "$config_file"'
assert_function_contains secure_app_config_permissions bootstrap.sh 'chmod 0640 "$config_file"'
assert_function_contains write_config_file install.sh 'secure_app_config_permissions'
assert_function_contains require_runtime install.sh 'require_command git'
assert_function_contains verify_containers install.sh 'local timeout_seconds=300'
...
assert_function_contains verify_production_readiness install.sh '[ "$(stat -c '\''%a'\'' "$ENV_FILE")" = "600" ]'
assert_function_contains verify_production_readiness install.sh '[ "$(stat -c '\''%a'\'' "$APP_CONFIG_FILE")" = "640" ]'
assert_function_contains verify_production_readiness install.sh 'docker compose exec -T "$service" gosu www-data test -r /var/www/html/config/.config.php'
assert_function_contains verify_production_readiness install.sh 'latest_db_version="$(latest_migration_version)"'
assert_function_contains verify_production_readiness install.sh "WHERE is_admin = 1 AND is_banned = 0 AND admin_role = 'owner';"
assert_function_contains verify_production_readiness install.sh 'http://127.0.0.1/healthz'
assert_function_contains resume_installation install.sh 'run_init_command Migration latest'
assert_function_contains resume_installation install.sh 'run_init_command Tool importSetting'
assert_function_contains resume_installation install.sh 'sync_geoip_defaults'
assert_function_contains resume_installation install.sh 'secure_app_config_permissions'
...
assert_function_order resume_installation install.sh 'run_init_command Migration latest' 'run_init_command Tool importSetting'
assert_function_order resume_installation install.sh 'run_init_command Tool importSetting' 'ensure_admin_for_resume'
assert_function_order resume_installation install.sh 'sync_geoip_defaults' 'secure_app_config_permissions'
...
assert_function_contains ensure_admin_for_resume install.sh 'ensure_admin_owner'
assert_function_contains create_admin install.sh 'ensure_admin_owner'
assert_file_contains src/Command/Tool.php 'public function ensureAdminOwner(): void'
assert_file_contains src/Command/Tool.php 'AdminPermissionService::ensureActiveOwner()'
assert_function_contains resume_installation install.sh 'verify_production_readiness'
assert_function_contains resume_installation install.sh 'verify_https'
assert_function_order resume_installation install.sh 'verify_containers' 'verify_production_readiness'
assert_function_order resume_installation install.sh 'verify_production_readiness' 'verify_https'
...
assert_function_contains main install.sh 'verify_production_readiness'
assert_function_order main install.sh 'verify_containers' 'verify_production_readiness'
assert_function_order main install.sh 'verify_production_readiness' 'verify_https'
