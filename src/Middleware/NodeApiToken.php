...
final class NodeApiToken implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($token === null) {
            return $this->error($request, 'Missing node token', 'AUTH_MISSING_TOKEN');
        }

        if (! $this->allowed('node_api_ip', $this->clientIp($request))
            || ! $this->allowed('node_api_key', hash('sha256', $token))) {
            return $this->error($request, 'Node API rate limit exceeded', 'RATE_LIMITED', 429);
        }
        ...
        return $handler->handle($request->withAttribute('xnode_node_id', (int) $tokenRecord->node_id));
    }
    ...
    private function extractToken(ServerRequestInterface $request): ?string
    {
        ...
        $debugToken = trim($request->getHeaderLine('X-Node-Token'));
